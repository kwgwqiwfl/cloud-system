...
public interface MlIcpMapper extends MyIdableMapper<MlIcp> {
    ...
    @Update("update ml_icp set query_count = #{queryCount} where domains = #{domain}")
    int updateByDomain(MlIcp icp);

    @Select("select * from ml_icp where domains = #{domain}")
    MlIcp selectByDomain(@Param("domain") String domain);

    // ===================== 修复版：XML特殊字符已转义 =====================
    @Insert("<script>"
            + "INSERT INTO ml_icp (domains, create_time, update_time) VALUES "
