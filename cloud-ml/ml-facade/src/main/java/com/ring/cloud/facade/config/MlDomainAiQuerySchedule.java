...
public class MlDomainAiQuerySchedule {
    ...
    public void execute() {
        ...
        try {
            ...
            WsUtil.push(WsMessageType.SCHEDULE_TASK, "域名AI查询开始");
            ...
            WsUtil.push(WsMessageType.SCHEDULE_TASK, "域名AI任务完成。耗时："+costs+"ms  成功次数："+success);
        } catch (Throwable e) {
            long fail = failCount.incrementAndGet();
            long cost = System.currentTimeMillis() - start;
            log.error("域名AI查询统计任务失败 → 耗时：{} ms  失败次数：{}  错误信息：{}", cost, fail, e.getMessage());
            WsUtil.push(WsMessageType.SCHEDULE_TASK, "域名AI查询失败!! 信息："+e.getMessage()+"  失败次数："+fail);
        }
    }
}