...
import com.ring.cloud.user.mybatis.mapper.SysUserRoleMapper;
...
@Service
@Transactional
public class SysUserRoleServiceImpl extends EntityClassServiceImpl<SysUserRole> implements SysUserRoleService {

    @Autowired
    private SysUserRoleMapper mapper;

    @Override
    public MyIdableMapper<SysUserRole> getMyIdableMapper() {
        return mapper;
    }

    @Override
    public PageResult<SysUserRole> pageList(CommonPageQuery query) {
        int offset = (query.getPageNum() - 1) * query.getPageSize();
        List<SysUserRole> list = mapper.pageList(query.getKey(), offset, query.getPageSize());
