...
public interface MlDomainMapper extends MyIdableMapper<MlDomain> {
    ...
    @Update("update ml_domain set query_count = #{queryCount} where domains = #{domain}")
    int updateByDomain(MlDomain domain);

    @Select("select * from ml_domain where domains = #{domain}")
    MlDomain selectByDomain(@Param("domain") String domain);

    // ===================== 修复版：XML特殊字符全部转义 =====================
    @Insert("<script>"
            + "INSERT INTO ml_domain (domains, create_time, update_time) VALUES "
